...
lbl = Label(window, text="Saisissez la cryptomonnaie à observer (BTC, ETH, ADA) : ")
lbl.grid(column=0, row=0)
crypto = Entry(window,width=10)
crypto.grid(column=2, row=0)

lbl = Label(window, text="Saisissez le nombre de derniers jours pour les prix existants (100, 150, 200 ...500...) : ")
lbl.grid(column=0, row=1)
last_days = Entry(window,width=10)
last_days.grid(column=2, row=1)

lbl = Label(window, text="Saisissez le nombre de jours de prédiction (5, 10, 15 ..., 30...) : ")
lbl.grid(column=0, row=2)
nb_prediction = Entry(window,width=10)
nb_prediction.grid(column=2, row=2)

def clicked():
    print(crypto.get())

btn = Button(window, text="Lancer la prédiction", command=clicked)

btn.grid(column=4, row=4)

window.mainloop()

# symb = input("Saisissez la cryptomonnaie à observer (BTC, ETH, ADA) : ").upper()
# lim = int(input("Saisissez le nombre de derniers jours pour les prix existants (100, 150, 200 ...500...) : "))
# num = int(input("Saisissez le nombre de jours de prédiction (5, 10, 15 ..., 30...) : "))
